...
## 5. ניסוי השחלפת מרכיבים (`Ablation Study`) — אסטרטגיית חלוקה ומודל הטמעות
...
### 5.4 ניסוי ב׳ — השחלפת מודל ההטמעות

בניסוי נפרד, הוחלף **רק** מודל ההטמעות תוך שמירה על 447 קטעי `section_aware` ו-50 שאלות הזהב הזהות.
`gemini-embedding-001` (768 ממדים, API) לעומת `multilingual-e5-large` (1024 ממדים, מקומי).
במקום תחיליות `"passage: "` / `"query: "` של `e5`, הוגדרו `task_type=RETRIEVAL_DOCUMENT` / `RETRIEVAL_QUERY`.
...
### 5.5 הערכת איכות תשובות (ידנית)

עשר תשובות נבדקו ידנית מול התשובה הצפויה בקבוצת הזהב:

| # | סוג שאלה | סיווג | הערה |
|---|---|---|---|
| 1 | מספרי (דמי השתתפות) | Incorrect | מודל אמר 0, אך התשובה הצפויה היא 90 ש״ח |
| 2 | מספרי (גבול אחריות) | Correct | התאמה מדויקת: 2,833,953 ש״ח |
| 3 | תנאי / עובדתי | Correct | כל התנאים התאימו (גיל 21+, רישיון 1+ שנה, תוקף בישראל) |
| 4 | מספרי (סכום ביטוח) | Incorrect | המודל אמר שהמידע לא זמין, אך התשובה היא 1,133 ש״ח |
| 5 | מספרי (פרמיה) | Incorrect | המודל אמר שלא מצא את הפרמיה, אך התשובה היא 58 ש״ח |
| 6 | מספרי (פרמיה) | Incorrect | המודל אמר אין פרמיה נפרדת (כלול בכיסוי), אך התשובה הצפויה היא 274 ש״ח — שגיאה מדויקת |
| 7 | תנאים מורכבים | Correct | תשובה מקיפה וערוכה היטב על תנאי הרכב החלופי בתרחישים שונים |
| 8 | עובדתי | Correct | התאמה מדויקת של התנאי המוקדם |
| 9 | השלכות חוזיות | Correct | תשובה מפורטת וכוללת על ההשלכות של מידע לא כנה |
| 10 | עובדתי | Correct | התאמה מדויקת |

**סיכום:** 6/10 Correct, 0/10 Partially correct, 4/10 Incorrect, 0/10 Hallucinated.
