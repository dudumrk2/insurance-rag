...
### ד. ניסוי השחלפת מודל ההטמעות (`gemini-embedding-001` לעומת `e5-large`)

#### מטרת הניסוי

ניסוי השחלפה עצמאי שבוצע לאחר השלמת הפרויקט האקדמי, לצורך תכנון האינטגרציה ב-`ai-wealth-monitor`. השאלה: האם `gemini-embedding-001` (מודל ענן של Google) יכול להחליף את `multilingual-e5-large` (מודל מקומי כבד, ~3GB) מבלי לפגוע באיכות השליפה בעברית?

#### מתודולוגיה

- **אותם קטעים בדיוק** — 447 קטעי `section_aware` (הגרסה המנצחת מהניסוי העיקרי).
- **החלפה יחידה** — רק מודל ההטמעות הוחלף; שאר ה-`pipeline` (החלוקה, קבוצת הזהב, מדדי ההערכה) נשאר זהה.
- **מימד הפלט** — `output_dimensionality=768` (לעומת 1024 ב-`e5-large`).
- **`task_type`** — `RETRIEVAL_DOCUMENT` לקטעים בעת אינדוקס; `RETRIEVAL_QUERY` לשאלות בעת שליפה (מחליף את תחיליות `"passage: "` / `"query: "` של `e5`).
- **50 שאלות הזהב** — אותה `gold_set.jsonl`, `top_k=5`.

ראה: `eval/run_eval_gemini.py`

#### תוצאות

| מודל | Hit@1 | Hit@3 | Hit@5 | MRR |
|---|---|---|---|---|
| **`gemini-embedding-001` (768d)** ⭐ | **0.500** | **0.740** | **0.800** | **0.615** |
| `multilingual-e5-large` (1024d, baseline) | — | — | 0.720 | 0.534 |
| **∆ (gemini − e5)** | — | — | **+0.080** | **+0.081** |

*ה-baseline של `e5` (0.720/0.534) מגיע מריצה עצמאית נפרדת של `run_eval.py` — שונה מעט מהתוצאות בסעיף 5.2 (0.740/0.529) בשל שונות בין ריצות ב-HNSW המקורב של ChromaDB.*

#### מסקנה

`gemini-embedding-001` עולה על `e5-large` ב-**+11% Hit@5** ו-**+15% MRR** על עברית — תוך שימוש בממד קטן יותר (768 לעומת 1024) וללא צורך ב-`sentence-transformers` / `torch` מקומי. הממצא מחזק את הבחירה ב-`gemini-embedding-001` לאינטגרציה ב-`ai-wealth-monitor`.
